import { listUsers, createUser, changeRole, changeStatus, AdminUser, CreateUserPayload, ChangeRolePayload, ChangeStatusPayload } from './adminUsers';
import api from './api';
...
describe('AdminUsers Service', () => {
  ...
  describe('changeRole', () => {
    ...
    it('deve tratar erro na alteração de role', async () => {
      ...
      await expect(changeRole(userId, payload, masterKey)).rejects.toThrow('Failed to change role');
    });
  });
  ...
  describe('changeStatus', () => {
    ...
    it('deve tratar erro na alteração de status', async () => {
      ...
      await expect(changeStatus(userId, payload)).rejects.toThrow('Failed to change status');
    });
  });
});