...
describe('EntryForm', () => {
  ...
  it('preenche o formulário com dados do entry quando fornecido', () => {
    ...
    expect(screen.getByDisplayValue(entry.amount.toString())).toBeInTheDocument();
  ...
  it('submete formulário com dados válidos', async () => {
    ...
    fireEvent.change(descriptionInput, { target: { value: 'Teste de lançamento' } });
    fireEvent.change(categorySelect, { target: { value: '1' } });
    fireEvent.click(submitButton);
  ...
  it('limpa erro quando formulário é resubmetido', async () => {
    ...
    expect(screen.queryByText('O valor deve ser maior que zero.')).not.toBeInTheDocument();
  });
  ...
  it('reseta formulário quando entry muda de editando para novo', () => {
    const { rerender } = render(<EntryForm {...defaultProps} entry={mockEntries[0]} />);
    
    // Verifica se está preenchido com dados do entry
    expect(screen.getByDisplayValue(mockEntries[0].description)).toBeInTheDocument();
