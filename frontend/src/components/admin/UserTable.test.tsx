...
describe('UserTable', () => {
  it('mostra skeleton enquanto carrega', () => {
    render(<UserTable users={[]} loading={true} actionId={null} onToggleStatus={()=>{}} onPromote={()=>{}} onDemote={()=>{}} />);
    expect(screen.getAllByRole('row').length).toBeGreaterThan(1); // header + skeleton rows
  });
  it('mostra empty state', () => {
    render(<UserTable users={[]} loading={false} actionId={null} onToggleStatus={()=>{}} onPromote={()=>{}} onDemote={()=>{}} />);
    expect(screen.getByText(/Nenhum usuário encontrado/i)).toBeInTheDocument();
  });
  it('renderiza usuário', () => {
    render(<UserTable users={[baseUser]} loading={false} actionId={null} onToggleStatus={()=>{}} onPromote={()=>{}} onDemote={()=>{}} />);
