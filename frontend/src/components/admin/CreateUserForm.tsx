...
export const CreateUserForm: React.FC<Props> = ({
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      await onCreate(
        ...
        name,
        role,
        role === "ADMIN" ? masterKey : undefined,
      );
      setEmail("");
      setName("");
      setRole("USER");
  ...
  return (
    <form
      onSubmit={submit}
      className="grid md:grid-cols-5 gap-3 items-end bg-gray-50 p-4 rounded border"
    >
      ...
      {error && (
        <div className="md:col-span-5 text-xs text-red-600">{error}</div>
      )}
    </form>
  );
};
