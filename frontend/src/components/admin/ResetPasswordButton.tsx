...
export const ResetPasswordButton: React.FC<Props> = ({
  user,
  loading,
  onResetPassword,
  currentUserRole,
  ...
  const handleReset = async () => {
    ...
    try {
      ...
      alert(
        `Senha resetada com sucesso! Email enviado para ${user.email}. A senha temporária expira em 24 horas.`,
      );
  ...
  return (
    <button
      ...
      title={
        hasValidEmail
          ? "Resetar senha e enviar nova senha por email"
          : "Usuário não possui email válido"
      }
    >
      <Key size={12} />
