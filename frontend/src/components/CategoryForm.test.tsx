...
describe('CategoryForm', () => {
  ...
  it('valida nome obrigatório', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText('O nome da categoria é obrigatório')).toBeInTheDocument();
    });
  ...
  it('adiciona subcategoria quando botão é clicado', async () => {
    ...
    const subcategoryInput = screen.getByPlaceholderText('Adicionar subcategoria');
    const addButton = screen.getByRole('button', { name: /adicionar/i });
    
    fireEvent.change(subcategoryInput, { target: { value: 'Nova Subcategoria' } });
  ...
  it('adiciona subcategoria quando Enter é pressionado', async () => {
    ...
    fireEvent.change(subcategoryInput, { target: { value: 'Subcategoria Enter' } });
  ...
  it('não adiciona subcategoria duplicada', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText('Esta subcategoria já existe')).toBeInTheDocument();
    });
  ...
  it('limpa erro quando nova subcategoria válida é adicionada', async () => {
    ...
    await waitFor(() => {
      expect(screen.queryByText('Esta subcategoria já existe')).not.toBeInTheDocument();
    });
  ...
  it('reseta formulário quando categoria muda de editando para nova', () => {
    const { rerender } = render(<CategoryForm {...defaultProps} category={mockCategories[0]} />);
