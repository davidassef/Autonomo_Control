...
const CategoryDistributionChart: React.FC<CategoryDistributionChartProps> = ({
  ...
  const categoryData = useMemo(() => {
    ...
    filteredEntries.forEach((entry) => {
      ...
      if (!categorySums[categoryId]) {
        ...
        categoryColors[categoryId] =
          ...
          defaultColors[
            Object.keys(categoryColors).length % defaultColors.length
          ];
      }
    ...
    if (sortedCategoryIds.length > 5) {
      const otherSum = sortedCategoryIds
        .slice(5)
        .reduce((sum, id) => sum + categorySums[id], 0);
      labels.push("Outros");
      data.push(otherSum);
    ...
    return (
      <div className="bg-white shadow rounded-lg p-4 h-60 flex items-center justify-center">
        <p className="text-gray-500 text-center">
          Nenhum {type === "EXPENSE" ? "despesa" : "receita"} no período
          selecionado.
        </p>
      </div>
    );
