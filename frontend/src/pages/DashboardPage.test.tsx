...
jest.mock('../components/charts/MonthlyEvolutionChart', () => {
  return function MockMonthlyEvolutionChart({ monthlySummaries, isLoading }: any) {
    if (isLoading) return <div data-testid="monthly-chart-loading">Carregando gráfico mensal...</div>;
    return (
...
jest.mock('../components/charts/CategoryDistributionChart', () => {
  return function MockCategoryDistributionChart({ entries, type, isLoading }: any) {
    if (isLoading) return <div data-testid={`category-chart-${type.toLowerCase()}-loading`}>Carregando gráfico...</div>;
    return (
...
describe('DashboardPage', () => {
  ...
  it('deve exibir loading nos cards quando isSummaryLoading é true', () => {
    ...
    const skeletonElements = loadingElements.filter(el => el.classList.contains('animate-pulse'));
  ...
  it('deve renderizar os gráficos', () => {
    ...
    expect(screen.getByTestId('category-distribution-chart-expense')).toBeInTheDocument();
    expect(screen.getByTestId('category-distribution-chart-income')).toBeInTheDocument();
  });
  ...
  it('deve exibir loading nos gráficos quando necessário', () => {
    ...
    expect(screen.getByTestId('category-chart-expense-loading')).toBeInTheDocument();
    expect(screen.getByTestId('category-chart-income-loading')).toBeInTheDocument();
  });
  ...
  it('deve exibir ícones corretos para receitas e despesas', () => {
    ...
    const incomeIcons = screen.getAllByRole('generic').filter(el => 
      el.classList.contains('bg-green-100') && el.classList.contains('text-green-600')
    );
    const expenseIcons = screen.getAllByRole('generic').filter(el => 
      el.classList.contains('bg-red-100') && el.classList.contains('text-red-600')
    );
