...
jest.mock("../components/CategoryList", () => {
  return function MockCategoryList({
    ...
    if (isLoading)
      return (
        <div data-testid="category-list-loading">Carregando categorias...</div>
      );
...
jest.mock("../components/CategoryForm", () => {
  return function MockCategoryForm({
    isOpen,
    onClose,
    onSave,
    category,
...
describe("CategoriesPage", () => {
  ...
  it('deve alternar para "Receitas" ao clicar no botão', () => {
    ...
    expect(receitasButton).toHaveClass(
      "bg-green-100 text-green-700 font-medium",
    );
    expect(screen.getByText("Despesas")).toHaveClass(
  ...
  it("deve fechar modal após salvar com sucesso", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.queryByTestId("category-form-modal"),
