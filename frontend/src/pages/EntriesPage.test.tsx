...
import { mockEntries, mockCategories, resetAllMocks } from '../utils/test-utils';
...
jest.mock('../components/EntryForm', () => {
  return function MockEntryForm({ isOpen, onClose, onSave, entry, categories }: any) {
...
jest.mock('../components/EntryList', () => {
  return function MockEntryList({ entries, isLoading, onEdit, onDelete }: any) {
    if (isLoading) return <div data-testid="entry-list-loading">Carregando lançamentos...</div>;
    
    return (
      <div data-testid="entry-list">
        <div data-testid="entries-count">{entries?.length || 0} lançamentos</div>
        {entries?.map((entry: any) => (
          <div key={entry.id} data-testid={`entry-${entry.id}`}>
            <span>{entry.description}</span>
            <button onClick={() => onEdit(entry)} data-testid={`edit-${entry.id}`}>Editar</button>
            <button onClick={() => onDelete(entry.id)} data-testid={`delete-${entry.id}`}>Excluir</button>
          </div>
        ))}
      </div>
    );
...
jest.mock('../components/EntryFilters', () => {
  return function MockEntryFilters({ filters, categories, isLoading, onFilterChange }: any) {
    return (
      <div data-testid="entry-filters">
        ...
        />
        <div data-testid="filter-categories-count">{categories?.length || 0} categorias</div>
      </div>
    );
  };
});

const mockUseEntries = useEntriesHook.useEntries as jest.MockedFunction<typeof useEntriesHook.useEntries>;
...
describe('EntriesPage', () => {
  ...
  it('deve renderizar a lista de lançamentos', () => {
    ...
    expect(screen.getByTestId('entries-count')).toHaveTextContent('3 lançamentos');
  });
  ...
  it('deve exibir mensagem de erro quando houver erro', () => {
    ...
    expect(screen.getByText('Erro ao carregar lançamentos')).toBeInTheDocument();
  });
  ...
  it('deve chamar deleteEntry ao confirmar exclusão', async () => {
    ...
    await waitFor(() => {
      expect(global.confirm).toHaveBeenCalledWith('Tem certeza que deseja excluir este lançamento?');
    });
  ...
  it('deve passar categorias para os componentes', () => {
    ...
    expect(screen.getByTestId('filter-categories-count')).toHaveTextContent('2 categorias');
    
    // Abre modal para verificar se categorias são passadas
    const newEntryButton = screen.getByText('Novo Lançamento');
    fireEvent.click(newEntryButton);
    
    expect(screen.getByTestId('categories-count')).toHaveTextContent('2 categorias');
  });
  ...
  it('deve inicializar filtros com datas do mês atual', () => {
    ...
    const expectedStartDate = new Date(currentDate.getFullYear(), currentDate.getMonth(), 1).toISOString().split('T')[0];
    const expectedEndDate = new Date(currentDate.getFullYear(), currentDate.getMonth() + 1, 0).toISOString().split('T')[0];
    
    expect(mockUseEntries).toHaveBeenCalledWith(expect.objectContaining({
  ...
  it('deve tratar erros ao salvar lançamento', async () => {
    ...
    await waitFor(() => {
      expect(consoleSpy).toHaveBeenCalledWith('Error saving entry:', expect.any(Error));
    });
