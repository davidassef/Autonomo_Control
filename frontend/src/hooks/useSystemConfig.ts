...
export function useSystemConfig(): UseSystemConfigReturn {
  ...
  const loadConfigs = useCallback(
    async (category?: string, publicOnly?: boolean) => {
      try {
        setLoading(true);
        ...
        const errorMessage =
          error.response?.data?.detail || "Erro ao carregar configurações";
  ...
  const loadConfigsByCategory = useCallback(
    async (category: string) => {
      try {
        setLoading(true);
        const configs =
          await systemConfigService.getConfigsByCategory(category);
        setState((prev) => ({ ...prev, configs, loading: false, error: null }));
      } catch (error: any) {
        const errorMessage =
          error.response?.data?.detail ||
          "Erro ao carregar configurações da categoria";
        setError(errorMessage);
        toast.error(errorMessage);
      }
    },
  ...
  const updateMultipleConfigs = useCallback(
    async (configs: { [key: string]: any }): Promise<boolean> => {
      try {
        setUpdating(true);
        ...
        const errorMessage =
          error.response?.data?.detail || "Erro ao atualizar configurações";
        setError(errorMessage);
        toast.error(errorMessage);
        return false;
      }
  ...
  const resetToDefaults = useCallback(async (): Promise<boolean> => {
    try {
      ...
      const errorMessage =
        error.response?.data?.detail || "Erro ao resetar configurações";
      setError(errorMessage);
      toast.error(errorMessage);
      return false;
    }
  ...
  const initializeDefaults = useCallback(async (): Promise<boolean> => {
    try {
      ...
      const errorMessage =
        error.response?.data?.detail || "Erro ao inicializar configurações";
      setError(errorMessage);
      toast.error(errorMessage);
      return false;
    }
