...
export const useSystemReports = (): UseSystemReportsReturn => {
  ...
  const [isLoadingFinancialOverview, setIsLoadingFinancialOverview] =
    useState(false);
  const [isLoadingHealthMetrics, setIsLoadingHealthMetrics] = useState(false);
  const [isLoadingEngagementReport, setIsLoadingEngagementReport] =
    useState(false);
  ...
  const [usageStatistics, setUsageStatistics] =
    useState<SystemUsageStatistics | null>(null);
  ...
  const [engagementReport, setEngagementReport] =
    useState<UserEngagementReport | null>(null);
  ...
  const fetchUsageStatistics = useCallback(async (days: number = 30) => {
    ...
    try {
      ...
      const errorMessage =
        err?.response?.data?.detail || "Erro ao carregar estatísticas de uso";
      setError(errorMessage);
      toast.error(errorMessage);
    } finally {
  ...
  const fetchHealthMetrics = useCallback(async () => {
    ...
    try {
      ...
      const errorMessage =
        err?.response?.data?.detail || "Erro ao carregar métricas de saúde";
      setError(errorMessage);
      toast.error(errorMessage);
    } finally {
  ...
  const fetchEngagementReport = useCallback(async (days: number = 30) => {
    ...
    try {
      ...
      const errorMessage =
        err?.response?.data?.detail ||
        "Erro ao carregar relatório de engajamento";
      setError(errorMessage);
      toast.error(errorMessage);
    } finally {
  ...
  const fetchDashboardData = useCallback(async () => {
    ...
    try {
      ...
      const errorMessage =
        err?.response?.data?.detail || "Erro ao carregar dados do dashboard";
      setError(errorMessage);
      toast.error(errorMessage);
    } finally {
