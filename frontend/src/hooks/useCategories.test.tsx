...
describe('useCategories Hook', () => {
  ...
  it('deve adicionar nova categoria', async () => {
    ...
    expect(result.current.categories).toEqual([mockCategories[0], ...mockCategories]);
  });
  ...
  it('deve excluir categoria', async () => {
    ...
    expect(result.current.categories).toEqual(mockCategories.filter(c => c.id !== '1'));
  });
  ...
  it('deve definir isLoading como true durante operações', async () => {
    ...
    mockCategoryService.create.mockImplementation(() => 
      new Promise(resolve => setTimeout(() => resolve(mockCategories[0]), 100))
    );
  ...
  it('deve atualizar estado corretamente após atualizar categoria', async () => {
    ...
    await act(async () => {
      await result.current.updateCategory('1', { name: 'Categoria Atualizada' });
    });
