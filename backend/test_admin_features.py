...
class AdminTester:
    ...
    def test_user_management(self):
        ...
        try:
            # Listar usuários
            response = requests.get(f"{BASE_URL}/admin/users", headers=self.get_headers())
            ...
            if response.status_code == 200:
                ...
                response = requests.post(f"{BASE_URL}/admin/users/{test_user_id}/block", headers=self.get_headers())
                print(f"Bloquear usuário - Status: {response.status_code}")
                
                # Testar desbloqueio de usuário
                response = requests.post(f"{BASE_URL}/admin/users/{test_user_id}/unblock", headers=self.get_headers())
                print(f"Desbloquear usuário - Status: {response.status_code}")
                
                # Testar reset de senha
                response = requests.post(f"{BASE_URL}/admin/users/{test_user_id}/reset-password", headers=self.get_headers())
    ...
    def test_audit_logs(self):
        ...
        try:
            ...
            response = requests.get(f"{BASE_URL}/admin/audit-logs", headers=self.get_headers())
            ...
            if self.user_id:
                response = requests.get(f"{BASE_URL}/admin/audit-logs?user_id={self.user_id}", headers=self.get_headers())
                print(f"Logs por usuário - Status: {response.status_code}")
            
            # Buscar logs por ação
            response = requests.get(f"{BASE_URL}/admin/audit-logs?action=login", headers=self.get_headers())
    ...
    def test_system_reports(self):
        ...
        try:
            # Relatório de uso
            response = requests.get(f"{BASE_URL}/admin/reports/usage", headers=self.get_headers())
            ...
            response = requests.get(f"{BASE_URL}/admin/reports/stats", headers=self.get_headers())
    ...
    def test_system_config(self):
        ...
        try:
            ...
            response = requests.get(f"{BASE_URL}/system-config/", headers=self.get_headers())
            ...
            response = requests.post(f"{BASE_URL}/system-config/initialize", headers=self.get_headers())
            ...
            response = requests.put(f"{BASE_URL}/system-config/", json=config_data, headers=self.get_headers())
