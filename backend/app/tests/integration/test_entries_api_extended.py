...
def test_create_entry_with_gross_amount_calculation(
    test_client, auth_headers, sample_category
):
    """
...
def test_read_entries_with_date_filters(
    test_client, auth_headers, test_db, sample_user, sample_category
):
    """
    ...
    response = test_client.get(
        f"/api/v1/entries/?start_date={today}", headers=auth_headers
    )
    ...
    assert (
        len(
            [
                e
                for e in entries
                if e["description"] in ["Entrada hoje", "Entrada amanhã"]
            ]
        )
        >= 2
    )
    ...
    response = test_client.get(
        f"/api/v1/entries/?end_date={today}", headers=auth_headers
    )
    assert response.status_code == status.HTTP_200_OK
    entries = response.json()
    ...
    assert (
        len(
            [
                e
                for e in entries
                if e["description"] in ["Entrada ontem", "Entrada hoje"]
            ]
        )
        >= 2
    )
...
def test_read_entries_with_search_filter(test_client, auth_headers, sample_category):
    ...
    response = test_client.get(
        "/api/v1/entries/?search=freelance", headers=auth_headers
    )
    assert response.status_code == status.HTTP_200_OK
    entries = response.json()
...
def test_get_entries_summary_with_date_filters(
    test_client, auth_headers, sample_category
):
    """
    ...
    response = test_client.get(
        f"/api/v1/entries/summary?start_date={today}&end_date={today}",
...
def test_get_monthly_summary(test_client, auth_headers, sample_category):
    ...
    response = test_client.get(
        f"/api/v1/entries/summary/monthly/{today.year}/{today.month}",
        headers=auth_headers,
    )
    assert response.status_code == status.HTTP_200_OK
    summary = response.json()
...
def test_get_monthly_summary_invalid_month(test_client, auth_headers):
    ...
    """
    response = test_client.get(
        "/api/v1/entries/summary/monthly/2023/13", headers=auth_headers
    )
...
def test_get_monthly_metrics_with_year_default(
    test_client, auth_headers, sample_category
):
    """
...
def test_get_monthly_metrics_with_platform_filter(
    test_client, auth_headers, sample_category
):
    """
    ...
    response = test_client.get(
        f"/api/v1/entries/metrics/monthly?year={current_year}&platform=UBER",
        headers=auth_headers,
    )
    assert response.status_code == status.HTTP_200_OK
    metrics = response.json()
...
def test_update_entry_with_gross_amount_recalculation(
    test_client, auth_headers, test_db, sample_user, sample_category
):
    """
    ...
    update_data = {"gross_amount": 30.0, "platform_fee": 6.0, "tips_amount": 2.0}

    response = test_client.patch(
        f"/api/v1/entries/{entry_id}", json=update_data, headers=auth_headers
    )
    assert response.status_code == status.HTTP_200_OK
    updated_entry = response.json()
...
def test_update_entry_not_found(test_client, auth_headers):
    ...
    response = test_client.put(
        f"/api/v1/entries/{fake_id}", json=update_data, headers=auth_headers
    )
    assert response.status_code == status.HTTP_404_NOT_FOUND
    assert "Lançamento não encontrado" in response.json()["detail"]
