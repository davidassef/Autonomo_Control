...
class SystemConfigService:
    ...
    def __init__(self, db: Session):
        ...
        self.default_configs = {
            ...
            "email_from": {
                "value": "noreply@autonomocontrol.com",
                "type": "string",
                "category": "email",
                "public": False,
            },
    ...
    def get_all_configs(
        self, category: Optional[str] = None, public_only: bool = False
    ) -> Dict[str, Any]:
        """
    ...
    def get_config(self, key: str) -> Optional[Any]:
        ...
        try:
            config = (
                ...
                .filter(and_(SystemConfig.key == key, SystemConfig.is_active == True))
                .first()
            )
    ...
    def update_multiple_configs(
        self, configs: Dict[str, Any], user_id: int
    ) -> Dict[str, bool]:
        """
    ...
    def reset_to_defaults(self, user_id: int) -> bool:
        ...
        try:
            ...
            configs = (
                self.db.query(SystemConfig).filter(SystemConfig.is_active == True).all()
            )
                ...
                history.append(
                    {
                        ...
                        "updated_by": {
                            "name": (
                                ...
                                if config.updated_by_user
                                else None
                            ),
                            "email": (
                                config.updated_by_user.email
                                if config.updated_by_user
                                else None
                            ),
                        },
    ...
    def validate_config_value(self, key: str, value: Any) -> tuple[bool, str]:
        ...
        try:
            ...
            if key == "password_min_length" and (value < 4 or value > 50):
                return (
                    False,
                    "Comprimento mínimo da senha deve estar entre 4 e 50 caracteres",
                )
