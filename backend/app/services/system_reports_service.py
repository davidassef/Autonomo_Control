...
class SystemReportsService:
    ...
    def get_user_statistics(db: Session, days: int = 30) -> Dict[str, Any]:
        ...
        active_users = (
            db.query(func.count(func.distinct(Entry.user_id)))
            .filter(Entry.created_at >= start_date)
            .scalar()
            or 0
        )
        ...
        most_active_users = (
            db.query(User.email, User.name, func.count(Entry.id).label("entries_count"))
        ...
        return {
            ...
            "most_active_users": [
                {"email": email, "name": name, "entries_count": entries_count}
    ...
    def get_system_usage_statistics(db: Session, days: int = 30) -> Dict[str, Any]:
        ...
        total_entries = db.query(Entry).filter(Entry.created_at >= start_date).count()
        ...
        audit_logs_count = (
            db.query(AuditLog).filter(AuditLog.created_at >= start_date).count()
        )
    ...
    def get_financial_overview(db: Session, days: int = 30) -> Dict[str, Any]:
        ...
        monthly_evolution = (
            ...
            .filter(Entry.created_at >= twelve_months_ago)
            .group_by(
        ...
        top_categories = (
            ...
            .filter(Entry.created_at >= start_date)
            .group_by(Entry.category, Entry.type)
            .order_by(desc(func.sum(Entry.amount)))
            .limit(20)
            .all()
        )
    ...
    def get_system_health_metrics(db: Session) -> Dict[str, Any]:
        ...
        activity_24h = {
            ...
            "active_users": db.query(func.count(func.distinct(Entry.user_id)))
            .filter(Entry.created_at >= last_24h)
            .scalar()
            or 0,
        ...
        activity_7d = {
            ...
            "active_users": db.query(func.count(func.distinct(Entry.user_id)))
            .filter(Entry.created_at >= last_7d)
            .scalar()
            or 0,
            "new_users": db.query(User).filter(User.created_at >= last_7d).count(),
            "audit_logs": db.query(AuditLog)
            .filter(AuditLog.created_at >= last_7d)
            .count(),
        }
    ...
    def get_user_engagement_report(db: Session, days: int = 30) -> Dict[str, Any]:
        ...
        user_activity = (
            ...
            .outerjoin(
                Entry, and_(User.id == Entry.user_id, Entry.created_at >= start_date)
            )
            .group_by(User.id, User.email, User.name)
            .all()
        )
