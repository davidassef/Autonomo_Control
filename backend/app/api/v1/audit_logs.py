...
def get_audit_stats(
    ...
    user_stats = (
        db.query(AuditLog.performed_by, func.count(AuditLog.id).label("count"))
        .filter(AuditLog.created_at >= start_date)
    ...
    return {
        ...
        "actions": [
            {"action": action, "count": count} for action, count in actions_stats
        ],
        "resource_types": [
        ...
        "most_active_users": [
            {"user": user, "count": count} for user, count in user_stats
        ],
    }
