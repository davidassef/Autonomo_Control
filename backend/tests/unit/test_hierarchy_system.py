...
def test_hierarchy_system():
    ...
    try:
        ...
        db.query(User).filter(
            User.email.in_(
                ["master@empresa.com", "usuario@empresa.com", "usuario2@empresa.com"]
            )
        ...
        result = hierarchy_service.toggle_admin_visibility(
            master_user=master_user, target_admin=common_user, can_view=True
        )
        if result:
        ...
        for user in all_users:
            print(
                f"   - {user.email}: {user.role} (can_view_admins: {user.can_view_admins})"
            )
